...
const DRAW_GRID = (ctx, canvas, screenOrigin, GRAPH_DATA) => {
    ...
    while(scanning.x < screenOrigin.x + canvas.width/2){
        ...
        if(scanning.x % GRAPH_DATA.minor_space === 0){
            ...
            if(gridType === 'axis' || gridType === 'major'){ //here major and axis is verticle line
                let value = (printing.x - center.x) /GRAPH_DATA.major_space   
                SHOW_TEXT(ctx, value, GRAPH_DATA.label_font_size, printing.x + 5, center.y + 20)
            }
            ...
            if(scanning.y % GRAPH_DATA.major_space === 0){
                ...
                if(value != 0){
                    ...
                    if(screenOrigin.x < 0){
                        ...
                        newPositionY = newPositionY - GRAPH_DATA.label_font_size*(value+"").length + (value+"").length
                    }
                    SHOW_TEXT(ctx, value, GRAPH_DATA.label_font_size, newPositionY, printing.y - 5)
                }
    ...
    while(scanning.y < screenOrigin.y + canvas.height/2){
        ...
        if(scanning.y % GRAPH_DATA.minor_space === 0){
            ...
            let gridType = "minor"
            if(scanning.y === 0){ //if the grid line is axis
                gridType = "axis"
            }else if(scanning.y % GRAPH_DATA.major_space === 0){ // if the grid line is a major line(second most thick line)
                gridType = "major"
            }
            // console.log("Hello")
            MAKE_LINE(ctx, canvas, from, to , gridType)
            ...
            if(gridType === 'axis' || gridType === 'major'){ //if grid line is axis or major
                let value = (center.y - printing.y) / GRAPH_DATA.major_space //center is the center of screen // printing is the position with respect to the screen 
                if(value != 0)
                SHOW_TEXT(ctx, value, GRAPH_DATA.label_font_size, center.x + 5, printing.y - 5)
            }
    ...
    if(Math.abs(screenOrigin.y) > canvas.height/2){
        if(screenOrigin.y < 0)
        printing.y = printing.y-10;
        else if(screenOrigin.y > 0)
        printing.y = 20;
        // console.log("Outside")
        ...
        while(scanning.x < screenOrigin.x + canvas.width/2){
            ...
            if(scanning.x % GRAPH_DATA.major_space === 0){
                ...
                let value = (printing.x - center.x) /GRAPH_DATA.major_space
                if(value != 0)
                SHOW_TEXT(ctx, value, GRAPH_DATA.label_font_size, printing.x + 5, printing.y)
